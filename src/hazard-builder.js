...
export const HAZARD_PARTICIPATION_MODES = Object.freeze(["mandatory", "avoidable", "conditional", "reinforcement"]);
...
export class HazardCompositionStore {
  ...
  add(snapshot, { participation = "avoidable", condition = null, placement = "", expectedRevision = null, origin = "gm" } = {}) {
    this.check(expectedRevision); if (!HAZARD_PARTICIPATION_MODES.includes(participation)) throw Object.assign(new Error("Hazard participation mode is invalid."), { code: "invalid_participation_mode" }); if (snapshot?.identity?.complexity === "simple") createSimpleHazard(snapshot); if (this.draft.hazards?.some(item => item.id === snapshot.id)) throw Object.assign(new Error("That Hazard is already in the Encounter."), { code: "duplicate_component" }); this.record(); this.hazards.push(structuredClone(snapshot)); this.draft.hazards = [...(this.draft.hazards ?? []), { id: snapshot.id, contentID: snapshot.provenance?.catalogContentID ?? `hazard/custom/${snapshot.id}/current`, name: snapshot.identity.name, level: snapshot.identity.level, complexity: snapshot.identity.complexity, participation: { mode: participation, condition }, placement }]; this.commit(origin); return snapshot.id;
  }
  update(snapshot, { participation, condition, placement, expectedRevision = null, origin = "gm" } = {}) {
    this.check(expectedRevision);
    if (participation !== undefined && !HAZARD_PARTICIPATION_MODES.includes(participation)) throw Object.assign(new Error("Hazard participation mode is invalid."), { code: "invalid_participation_mode" });
    const next = createSimpleHazard(snapshot);
    const index = this.hazards.findIndex((item) => item.id === next.id);
    const encounterIndex = (this.draft.hazards ?? []).findIndex((item) => item.id === next.id);
    if (index < 0 || encounterIndex < 0) throw Object.assign(new Error("That Hazard is not in the Encounter."), { code: "unknown_component" });
    this.record();
    this.hazards[index] = structuredClone(next);
    const placed = { ...this.draft.hazards[encounterIndex], name: next.identity.name, level: next.identity.level };
    if (participation !== undefined) placed.participation = { mode: participation, condition: condition ?? placed.participation?.condition ?? null };
    if (placement !== undefined) placed.placement = placement;
    this.draft.hazards[encounterIndex] = placed;
    this.commit(origin);
    return next.id;
  }
  edit(...args) { return this.update(...args); }
...
export function createHazardBuilder({ root, hazard = createEmptySimpleHazard(), partyLevel = 1, participation = "avoidable", onMutation = () => {}, onAutosave = () => {}, onAddToEncounter = () => {}, onEditHazard = () => {}, onRemoveHazard = () => {} }) {
  ...
  const render = () => {
    ...
    const defenses = current.defenses ?? {};
    root.innerHTML = `<section class="hazard-builder"><header><p class="eyebrow">Simple Hazard · DM-owned</p><h2>${escapeHTML(identity.name || "Untitled Hazard")}</h2><span data-testid="hazard-readiness">${escapeHTML(result.status)}</span></header><div class="hazard-fields"><label>Name<input data-field="name" value="${escapeHTML(identity.name)}"></label><label>Level<input data-field="level" type="number" min="-1" max="20" value="${identity.level ?? 1}"></label><label>Type<select data-field="type">${HAZARD_TYPES.map((type) => `<option value="${type}" ${identity.type === type ? "selected" : ""}>${type}</option>`).join("")}</select></label><label>Traits<input data-field="traits" value="${escapeHTML((identity.traits ?? []).join(", "))}"></label><label>Description<textarea data-field="description">${escapeHTML(current.description)}</textarea></label><label>Detection DC<input data-field="detection" type="number" value="${detection.value ?? ""}"></label><label>Detection band<select data-field="detectionBand">${HAZARD_BANDS.map((band) => `<option value="${band}" ${detection.band === band ? "selected" : ""}>${band}</option>`).join("")}</select></label><label>Disable Methods <span>(one Skill|DC per line)</span><textarea data-field="disableMethods">${escapeHTML(methods)}</textarea></label><label>Trigger<textarea data-field="trigger">${escapeHTML(current.trigger)}</textarea></label><label>Effect<textarea data-field="effect">${escapeHTML(effect.text)}</textarea></label><label>Reset<textarea data-field="reset">${escapeHTML(current.reset ?? "")}</textarea></label><label>Participation<select data-field="participation">${HAZARD_PARTICIPATION_MODES.map((mode) => `<option value="${mode}" ${selectedParticipation === mode ? "selected" : ""}>${mode}</option>`).join("")}</select></label><label>Participation condition<textarea data-field="participationCondition">${escapeHTML(current.participation?.condition ?? "")}</textarea></label><label>Armor Class<input data-field="defenseAC" type="number" value="${defenses.ac ?? ""}"></label><label>Hardness<input data-field="defenseHardness" type="number" value="${defenses.hardness ?? ""}"></label><label>Hit Points<input data-field="defenseHP" type="number" value="${defenses.hp ?? ""}"></label><label><input data-field="oneUse" type="checkbox" ${current.oneUse ? "checked" : ""}> One-use Hazard</label></div><section class="hazard-benchmark" aria-live="polite"><p>${result.structuralErrors.length} structural error(s)</p><ul data-testid="hazard-structural-errors">${structuralMarkup || "<li>None</li>"}</ul><p>${result.holisticWarnings.length} design warning(s)</p><ul data-testid="hazard-design-warnings">${warningMarkup || "<li>None</li>"}</ul></section><footer><button type="button" data-action="add" ${result.isStructurallyReady ? "" : "disabled"}>Add to encounter</button><button type="button" data-action="edit" ${result.isStructurallyReady ? "" : "disabled"}>Save Hazard changes</button><button type="button" data-action="remove">Remove Hazard</button><button type="button" data-action="undo" ${history.length ? "" : "disabled"}>Undo</button><button type="button" data-action="redo" ${redoHistory.length ? "" : "disabled"}>Redo</button><span>Revision ${current.revision} · ${escapeHTML(current.provenance?.mutationOrigin ?? "gm")}</span></footer></section>`;
    root.querySelectorAll("[data-field]").forEach((field) => field.addEventListener("change", () => mutate((value) => { const key = field.dataset.field; if (key === "name") value.identity.name = field.value; else if (key === "level") value.identity.level = Number(field.value); else if (key === "type") value.identity.type = field.value; else if (key === "traits") value.identity.traits = field.value.split(",").map((item) => item.trim()).filter(Boolean); else if (key === "description" || key === "trigger" || key === "reset") value[key] = field.value; else if (key === "detection") value.detection.value = Number(field.value); else if (key === "detectionBand") value.detection.band = field.value; else if (key === "disableMethods") value.disableMethods = parseDisableMethods(field.value); else if (key === "oneUse") value.oneUse = field.checked; else if (key === "effect") value.effect.text = field.value; else if (key === "participation") { selectedParticipation = field.value; value.participation = { ...(value.participation ?? {}), mode: field.value }; } else if (key === "participationCondition") value.participation = { ...(value.participation ?? {}), condition: field.value }; else if (["defenseAC", "defenseHardness", "defenseHP"].includes(key)) value.defenses = { ...(value.defenses ?? {}), [key.replace("defense", "").toLowerCase()]: Number(field.value) }; }, "gm")));
    ...
    root.querySelector('[data-action="add"]').addEventListener("click", () => onAddToEncounter({ hazard: structuredClone(current), participation: { mode: selectedParticipation, condition: current.participation?.condition ?? null }, participationMode: selectedParticipation, xp: projectHazardXP(current.identity.level, partyLevel, "simple", selectedParticipation) }));
    root.querySelector('[data-action="edit"]').addEventListener("click", () => onEditHazard({ hazard: structuredClone(current), participation: { mode: selectedParticipation, condition: current.participation?.condition ?? null }, participationMode: selectedParticipation }));
    root.querySelector('[data-action="remove"]').addEventListener("click", () => onRemoveHazard({ hazard: structuredClone(current), participation: { mode: selectedParticipation, condition: current.participation?.condition ?? null } }));
  };
  render();
  return {
