...
function participantCardMarkup(group, draft, catalog) {
  ...
  const strikes = (detail.strikes ?? []).slice(0, 3).map(strike => `<li><strong>${escapeHtml(strike.name ?? "Strike")}</strong><span>+${escapeHtml(projectedStatistic(strike.attack, group, proficiencyWithoutLevel))} · ${escapeHtml(strike.damage?.[0]?.expression ?? strike.damage ?? "damage not recorded")}</span></li>`).join("");
  ...
  app.innerHTML = `
    <div class="shell">
      ...
      <div class="workspace">
        ...
        <aside class="command-rail"><section class="rail-panel"><div class="panel-heading"><div><p class="eyebrow">Party and budget</p><h2>Encounter target</h2></div><strong class="threat-mark">${escapeHtml(target.kind)}</strong></div><div class="party-grid"><label>Party level<input data-testid="party-level" data-field="effective_level" type="number" min="1" max="20" value="${party.effectiveLevel}"></label><label>Party size<input data-testid="party-size" data-field="size" type="number" min="1" max="8" value="${party.size}"></label></div><label>Threat target<select data-testid="threat-target" data-field="threat">${["trivial","low","moderate","severe","extreme","custom"].map((kind) => `<option value="${kind}" ${target.kind === kind ? "selected" : ""}>${kind[0].toUpperCase() + kind.slice(1)}</option>`).join("")}</select></label><label data-testid="custom-xp-field" class="${target.kind === "custom" ? "" : "hidden"}">Custom XP<input data-testid="custom-xp" data-field="custom_xp" type="number" min="0" value="${target.customXP ?? 0}"></label><div class="budget"><div class="budget-hero"><span>Peak active</span><strong>${escapeHtml(budget.peakActiveXP)} <small>/ ${escapeHtml(budget.constructionBudget)} XP</small></strong></div>${budgetMarkup(budget)}<p class="inferred">Inferred threat: <strong data-testid="inferred-threat">${escapeHtml(budget.inferredThreat)}</strong></p>${budget.warnings.map((warning) => `<p class="warning">${escapeHtml(warning)}</p>`).join("")}</div></section>
          <section class="rail-panel encounter-settings"><div class="panel-heading"><h2>Encounter</h2><button type="button" class="button-ghost" data-action="new-encounter">New</button></div><label>Title<input data-testid="encounter-title" data-field="title" value="${escapeHtml(draft.title)}"></label><div class="rail-actions"><button type="button" class="button-secondary" data-modal-open="npc" ${npcTarget ? "" : "disabled"}>NPC profiles</button><button type="button" class="button-secondary" data-modal-open="phase">Phases</button><button type="button" class="button-secondary" data-modal-open="transfer">Library</button></div></section>
          <section class="rail-panel activity-panel"><div class="panel-heading"><h2>Recent changes</h2><span class="connection-dot" title="${escapeHtml(uiState.webMCPStatus)}"></span></div><ul>${activities || "<li class=empty>No mutations yet.</li>"}</ul></section>
        </aside>
      </div>
