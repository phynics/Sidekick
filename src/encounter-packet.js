...
export function validatePacket(packet = createEmptyPacket()) {
  ...
  if (!list(packet.information?.gmSecret)) designWarnings.push(designWarning("information", "gmSecret", "No GM-secret information is documented."));
  ...
  if (!list(packet.runningGuidance?.adjudicationIssues)) designWarnings.push(designWarning("running_guidance", "adjudicationIssues", "Consider noting one likely adjudication question for the GM."));
...
function formFor(section, packet) {
  ...
  if (section === "information") return [listField("immediatelyApparent", "Immediately apparent", value.immediatelyApparent), listField("discoverable", "Discoverable", value.discoverable), listField("gmSecret", "GM secret", value.gmSecret)].join("");
