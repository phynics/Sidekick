...
function render({ asset, engine, catalog, catalogState = { query: "", results: null }, bridgeMessage = "", notice = "" }) {
    ...
    <main class="shell">
      ...
      <section class="panel brief-panel"><h2>Party Snapshot</h2>
        ...
        <section class="panel encounter-panel"><div class="panel-heading"><h2>Encounter Draft</h2><span class="badge" data-testid="readiness">${escapeHtml(snapshot.readiness?.status ?? "incomplete")}</span></div>
          ...
          <div class="controls"><button type="button" data-action="undo" ${snapshot.canUndo ? "" : "disabled"}>Undo</button><button type="button" data-action="redo" ${snapshot.canRedo ? "" : "disabled"}>Redo</button><button type="button" data-action="export-encounter">Export Encounter JSON</button><button type="button" data-action="export-encounter-zip">Export Encounter ZIP</button><label class="file-control">Import Encounter JSON<input type="file" accept="application/json,.json" data-action="import-encounter"></label><label class="file-control">Import Encounter ZIP<input type="file" accept="application/zip,.zip,.sidekickdm.zip" data-action="import-encounter-zip"></label><button type="button" data-action="print-encounter">Print Packet</button><button type="button" data-action="increment" class="legacy-control">Change Swift state</button></div><span class="sr-only">Swift-owned value <span data-testid="swift-value">${draft.swiftOwnedValue}</span></span>
  ...
  if (manualLocked) {
    app.querySelectorAll("input, select, textarea, button").forEach(control => {
      if (!control.matches('[data-action^="export-"], [data-action="print-encounter"]')) control.disabled = true;
    });
  }
}
