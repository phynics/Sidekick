...
const SAVE_HIGH = [8, 9, 10, 11, 12, 14, 15, 17, 18, 19, 21, 22, 24, 25, 26];
const SAVE_MODERATE = [5, 6, 7, 8, 9, 11, 12, 14, 15, 16, 18, 19, 21, 22, 23];
...
const ABILITY_KINDS = Object.freeze(["action", "reaction", "free_action", "passive"]);
const DEFENSE_FIELDS = Object.freeze(["ac", "fortitude", "reflex", "will", "hp"]);
const ENCOUNTER_ROLES = Object.freeze(["brute", "defender", "skirmisher", "sniper", "controller", "support", "ambusher", "leader", "solo_boss"]);

function statisticValue(statistic) {
  if (statistic && typeof statistic === "object") return Number(statistic.value);
...
export function normalizeOriginalCreature(value) {
  const defaults = createEmptyOriginalCreature();
  const creature = { ...defaults, ...structuredClone(value ?? defaults) };
  creature.identity = { ...defaults.identity, ...(creature.identity ?? {}) };
  creature.identity.traits = listValue(creature.identity.traits);
  creature.defenses = { ...defaults.defenses, ...(creature.defenses ?? {}) };
  creature.defenses.immunities = listValue(creature.defenses.immunities);
  creature.defenses.weaknesses = listValue(creature.defenses.weaknesses);
  creature.defenses.resistances = listValue(creature.defenses.resistances);
  creature.senses = listValue(creature.senses);
  creature.languages = listValue(creature.languages);
  creature.skills = creature.skills && typeof creature.skills === "object" && !Array.isArray(creature.skills) ? creature.skills : {};
  creature.speeds = Array.isArray(creature.speeds) ? Object.fromEntries(creature.speeds.map((speed) => [speed.type ?? "land", Number(speed.feet ?? speed.value)])) : creature.speeds && typeof creature.speeds === "object" ? creature.speeds : {};
  creature.strikes = listValue(creature.strikes).map((strike, index) => ({
    id: `strike_${index + 1}`, name: "", actionCost: 1, traits: [], attack: null, damage: [], effect: "",
    ...(strike ?? {}),
    traits: listValue(strike?.traits),
    damage: listValue(strike?.damage).map(damage => ({ expression: "", type: "", ...(damage ?? {}) })),
    effect: typeof strike?.effect === "string" ? strike.effect : ""
  }));
  creature.abilities = listValue(creature.abilities).map((ability, index) => ({
    id: `ability_${index + 1}`, name: "", kind: "action", actionCost: null, traits: [], trigger: null,
    requirements: null, target: null, range: null, area: null, resolution: null, damage: [], conditions: [],
    duration: null, frequency: null, effectText: "", ...(ability ?? {}),
    traits: listValue(ability?.traits),
    damage: listValue(ability?.damage).map(damage => ({ expression: "", type: "", ...(damage ?? {}) })),
    conditions: listValue(ability?.conditions),
    effectText: typeof ability?.effectText === "string" ? ability.effectText : ""
  }));
  creature.spellcastingBlocks = listValue(creature.spellcastingBlocks);
  creature.provenance = { ...defaults.provenance, ...(creature.provenance ?? {}) };
  return creature;
}

export function validateOriginalCreature(creature = createEmptyOriginalCreature()) {
  creature = normalizeOriginalCreature(creature);
  const structuralErrors = [], benchmarkDeviations = [], holisticWarnings = [];
  const required = [["identity.name", creature.identity?.name, "A Creature name is required."], ["identity.concept", creature.identity?.concept, "A Creature concept is required."], ["identity.size", creature.identity?.size, "A Creature size is required."], ["identity.roadmap", creature.identity?.roadmap, "A Creature Roadmap is required."], ["identity.traits", creature.identity?.traits?.length, "At least one Creature trait is required."], ["languages", creature.languages?.length, "At least one language is required."], ["speeds", Array.isArray(creature.speeds) ? creature.speeds.length : Object.keys(creature.speeds ?? {}).length, "At least one Speed is required."], ["tactics", creature.tactics, "Tactics are required."], ["morale", creature.morale, "Morale or an exit condition is required."]];
  ...
  if (creature.identity?.roadmap != null && !CREATURE_ROADMAPS.includes(creature.identity.roadmap)) structuralErrors.push(issue("invalid_roadmap", "identity.roadmap", "Creature Roadmap is not supported."));
  if (!ENCOUNTER_ROLES.includes(creature.identity?.encounterRole)) structuralErrors.push(issue("invalid_encounter_role", "identity.encounterRole", "Creature encounter role is not supported."));
  ...
  const check = (field, stat, table) => { if (!stat) { structuralErrors.push(issue("required", field, "This statistic is required.")); return; } const actual = statisticValue(stat); if (!(actual > 0)) structuralErrors.push(issue("invalid_value", field, "This statistic must be greater than zero.")); if (!CREATURE_BANDS.includes(stat.band)) { structuralErrors.push(issue("invalid_benchmark_band", `${field}.band`, "Statistic benchmark band is not supported.")); return; } if (!table) return; const expected = table[stat.band]; const deviation = actual < expected.minimum ? actual - expected.minimum : actual > expected.maximum ? actual - expected.maximum : 0; if (deviation) benchmarkDeviations.push({ statistic: field, band: stat.band, expected, actual, deviation }); };
  ...
  if (!strikes.length) structuralErrors.push(issue("required", "strikes", "At least one Strike or offensive action is required."));
  strikes.forEach((strike, index) => { const prefix = `strikes[${index}]`; if (!text(strike.name)) structuralErrors.push(issue("required", `${prefix}.name`, "A Strike name is required.")); if (![1, 2, 3].includes(Number(strike.actionCost))) structuralErrors.push(issue("invalid_action_cost", `${prefix}.action_cost`, "Strike action cost must be 1, 2, or 3.")); if (!strike.damage?.some((damage) => text(damage.expression))) structuralErrors.push(issue("required", `${prefix}.damage`, "A Strike needs at least one damage expression.")); check(`${prefix}.attack`, strike.attack, benchmarks?.attack); });
  (creature.abilities ?? []).forEach((ability, index) => { const prefix = `abilities.${ability.id ?? index}`; if (!text(ability.name)) structuralErrors.push(issue("required", `${prefix}.name`, "An ability name is required.")); if (ability.kind != null && !ABILITY_KINDS.includes(String(ability.kind))) structuralErrors.push(issue("invalid_ability_kind", `${prefix}.kind`, "Ability kind must be an action, reaction, free action, or passive.")); if (ability.actionCost != null && ![1, 2, 3].includes(Number(ability.actionCost))) structuralErrors.push(issue("invalid_action_cost", `${prefix}.action_cost`, "Ability action cost must be 1, 2, or 3.")); if (ability.resolution?.dc && benchmarks) check(`${prefix}.dc`, ability.resolution.dc, benchmarks.dc); if (ability.conditions?.length && !ability.frequency) holisticWarnings.push(issue("unlimited_control", prefix, "A control ability applies conditions without a frequency or other visible limit.")); });
...
function commaList(value) { return listValue(value).join(", "); }
function lineList(value) { return listValue(value).join("\n"); }
function parseCommaList(value) { return String(value ?? "").split(",").map((item) => item.trim()).filter(Boolean); }
function parseLineList(value) { return String(value ?? "").split(/\r?\n/).map((item) => item.trim()).filter(Boolean); }
function parseSkills(value) { return Object.fromEntries(parseLineList(value).map((line) => { const [name, rank] = line.split("|").map((item) => item.trim()); return [name, rank === undefined || rank === "" ? "" : Number(rank)]; }).filter(([name]) => name)); }
function skillsText(value) { return Object.entries(value ?? {}).map(([name, rank]) => `${name}|${rank ?? ""}`).join("\n"); }
function speedEntries(value) { return Object.entries(value ?? {}).filter(([name]) => name !== "land").map(([name, feet]) => `${name}|${feet}`).join("\n"); }
...
export function createCreatureBuilder({ root, creature = createEmptyOriginalCreature(), partyLevel = 1, submitLabel = "Add to encounter", onMutation = () => {}, onAutosave = () => {}, onAddToEncounter = () => {} }) {
  ...
  const render = () => {
    ...
    root.innerHTML = `<section class="creature-builder">
      <header class="builder-title"><div><p class="eyebrow">Original Creature · DM-owned</p><h2>${escapeHTML(fields.name || "Untitled Creature")}</h2></div><span class="builder-status" data-testid="creature-readiness">${escapeHTML(result.status)}</span></header>
    ...
    root.querySelector('[data-action="add-strike"]').addEventListener("click", () => mutate((value) => value.strikes.push(strikeTemplate(`strike_${value.strikes.length + 1}`))));
    root.querySelector('[data-action="add-ability"]').addEventListener("click", () => mutate((value) => value.abilities.push(abilityTemplate(`ability_${value.abilities.length + 1}`))));
    root.querySelectorAll('[data-action="remove-strike"]').forEach((button) => button.addEventListener("click", () => mutate((value) => value.strikes.splice(Number(button.dataset.index), 1))));
    root.querySelectorAll('[data-action="remove-ability"]').forEach((button) => button.addEventListener("click", () => mutate((value) => value.abilities.splice(Number(button.dataset.index), 1))));
    root.querySelector('[data-action="undo"]').addEventListener("click", () => { if (!history.length) return; redoHistory.push(structuredClone(current)); current = normalizeOriginalCreature(history.pop()); current.revision += 1; emit("gm"); render(); });
    root.querySelector('[data-action="redo"]').addEventListener("click", () => { if (!redoHistory.length) return; history.push(structuredClone(current)); current = normalizeOriginalCreature(redoHistory.pop()); current.revision += 1; emit("gm"); render(); });
  ...
  render();
  return { get creature() { return structuredClone(current); }, get revision() { return current.revision; }, get readiness() { return readiness(); }, render, setCreature(next, origin = "gm") { history.push(structuredClone(current)); current = normalizeOriginalCreature(next); current.revision = (Number(current.revision) || 0) + 1; redoHistory = []; emit(origin); render(); }, loadCreature(next, origin = "gm") { this.setCreature(next, origin); }, autosave() { return persistence(); }, destroy() { root.replaceChildren(); } };
}
