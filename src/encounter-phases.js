...
export function createEncounterPhaseEditor({ root, phase = createEmptyPhase(), participantGroups = [], hazards = [], onMutation = () => {}, onAutosave = () => {} }) {
  ...
  const mutate = (operation, origin = "gm") => { const before = clone(current); operation(current); current.revision = Number(current.revision ?? 0) + 1; history.push(before); redoHistory = []; onMutation({ command: "sidekickdm_upsert_phase", phase: clone(current), expected_phase_revision: current.revision - 1, origin }); onAutosave({ format: "sidekickdm-encounter-phases", format_version: 1, phase: clone(current), revision: current.revision, origin }); render(); };
  const render = () => { const result = readiness(); const errors = [...result.structuralErrors, ...result.designWarnings].map(item => `<li data-kind="${item.code.startsWith("unknown") || item.code === "required" ? "structural-error" : "design-warning"}">${escapeHTML(item.message)}</li>`).join(""); root.innerHTML = `<section class="encounter-phase-editor"><header><p class="eyebrow">Encounter Phase</p><h2>${escapeHTML(current.title || "Untitled Phase")}</h2><span data-testid="phase-readiness">${escapeHTML(result.status)}</span></header><label>Title<input data-field="title" value="${escapeHTML(current.title)}"></label><label>Trigger type<select data-field="triggerKind">${PHASE_TRIGGER_TYPES.map(kind => `<option value="${kind}" ${kind === current.trigger?.kind ? "selected" : ""}>${kind}</option>`).join("")}</select></label><label>Trigger explanation<textarea data-field="triggerExplanation">${escapeHTML(current.trigger?.explanation)}</textarea></label><label>Active Participant Group IDs<input data-field="participantIDs" value="${escapeHTML((current.participantIDs ?? []).join(", "))}"></label><label>Active Hazard IDs<input data-field="hazardIDs" value="${escapeHTML((current.hazardIDs ?? []).join(", "))}"></label><label>Terrain changes <span>(one item per line)</span><textarea data-field="terrainChanges">${escapeHTML((current.terrainChanges ?? []).map(item => item.description ?? item).join("\n"))}</textarea></label><label>Running guidance<textarea data-field="runningGuidance">${escapeHTML(current.runningGuidance)}</textarea></label><section aria-live="polite"><p>${result.structuralErrors.length} structural error(s), ${result.designWarnings.length} design warning(s)</p><ul>${errors}</ul></section><footer><button type="button" data-action="undo" ${history.length ? "" : "disabled"}>Undo</button><button type="button" data-action="redo" ${redoHistory.length ? "" : "disabled"}>Redo</button><span>Revision ${current.revision}</span></footer></section>`; root.querySelectorAll("[data-field]").forEach(field => field.addEventListener("change", () => mutate(value => { if (field.dataset.field === "title") value.title = field.value; else if (field.dataset.field === "triggerKind") value.trigger.kind = field.value; else if (field.dataset.field === "triggerExplanation") value.trigger.explanation = field.value; else if (field.dataset.field === "participantIDs") value.participantIDs = field.value.split(",").map(item => item.trim()).filter(Boolean); else if (field.dataset.field === "hazardIDs") value.hazardIDs = field.value.split(",").map(item => item.trim()).filter(Boolean); else if (field.dataset.field === "terrainChanges") value.terrainChanges = field.value.split("\n").map(description => ({ title: "", description: description.trim() })).filter(item => item.description); else if (field.dataset.field === "runningGuidance") value.runningGuidance = field.value; }))); root.querySelector('[data-action="undo"]').addEventListener("click", () => { if (!history.length) return; redoHistory.push(clone(current)); current = history.pop(); current.revision += 1; onMutation({ command: "sidekickdm_undo", expected_phase_revision: current.revision - 1, origin: "gm" }); render(); }); root.querySelector('[data-action="redo"]').addEventListener("click", () => { if (!redoHistory.length) return; history.push(clone(current)); current = redoHistory.pop(); current.revision += 1; onMutation({ command: "sidekickdm_redo", expected_phase_revision: current.revision - 1, origin: "gm" }); render(); }); };
  render();
  return { get phase() { return clone(current); }, get revision() { return current.revision; }, get readiness() { return readiness(); }, render, setPhase(next, origin = "gm") { mutate(value => Object.assign(value, clone(next)), origin); }, undo() { if (history.length) { redoHistory.push(clone(current)); current = history.pop(); current.revision += 1; render(); } }, redo() { if (redoHistory.length) { history.push(clone(current)); current = redoHistory.pop(); current.revision += 1; render(); } }, autosave() { return { format: "sidekickdm-encounter-phases", format_version: 1, phase: clone(current), revision: current.revision }; }, destroy() { root.replaceChildren(); } };
}
